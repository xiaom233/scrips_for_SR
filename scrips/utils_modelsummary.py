...
def get_model_flops_ignn(model, input_res_s, input_res, print_per_layer_stat=True,
                              input_constructor=None):
    assert type(input_res) is tuple, 'Please provide the size of the input image.'
    assert len(input_res) >= 3, 'Input image should have 3 dimensions.'
    flops_model = add_flops_counting_methods(model)
    ...
    if input_constructor:
        input = input_constructor(input_res)
        input_s = input_constructor(input_res_s)
        _ = flops_model(**input_s, **input)
    else:
        device = list(flops_model.parameters())[-1].device
        batch = torch.FloatTensor(1, *input_res).to(device)
        batch_s = torch.FloatTensor(1, *input_res_s).to(device)
        _ = flops_model(batch_s, batch)
...
def get_model_activation_ignn(model, input_res_s, input_res, input_constructor=None):
    assert type(input_res) is tuple, 'Please provide the size of the input image.'
    assert len(input_res) >= 3, 'Input image should have 3 dimensions.'
    activation_model = add_activation_counting_methods(model)
    activation_model.eval().start_activation_count()
    if input_constructor:
        input = input_constructor(input_res)
        input_s = input_constructor(input_res_s)
        _ = activation_model(**input_s, **input)
    else:
        device = list(activation_model.parameters())[-1].device
        batch = torch.FloatTensor(1, *input_res).to(device)
        batch_s = torch.FloatTensor(1, *input_res_s).to(device)
        _ = activation_model(batch_s, batch)
