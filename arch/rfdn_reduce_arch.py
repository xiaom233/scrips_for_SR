...
class RFDN_REDUCE(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        super(RFDN_REDUCE, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
        ...
        self.map1 = nn.Linear(num_feat, num_feat-1 * reduce_c)

        self.B2 = RFDB(in_channels=num_feat-1 * reduce_c, conv=self.conv, p=p)
        self.map2 = nn.Linear(num_feat-1 * reduce_c, num_feat - 2 * reduce_c)

        self.B3 = RFDB(in_channels=num_feat - 2 * reduce_c, conv=self.conv, p=p)
        self.map3 = nn.Linear(num_feat - 2 * reduce_c, num_feat - 3 * reduce_c)

        self.B4 = RFDB(in_channels=num_feat - 3 * reduce_c, conv=self.conv, p=p)
        self.map4 = nn.Linear(num_feat - 3 * reduce_c, num_feat - 4 * reduce_c)

        self.B5 = RFDB(in_channels=num_feat - 4 * reduce_c, conv=self.conv, p=p)
        self.map5 = nn.Linear(num_feat - 4 * reduce_c, num_feat - 5 * reduce_c)

        self.B6 = RFDB(in_channels=num_feat - 5 * reduce_c, conv=self.conv, p=p)
