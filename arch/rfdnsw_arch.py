...
class SW(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.25):
        ...
        self.attn_lr = nn.Parameter(torch.normal(mean=1, std=0.2, size=(1, num_feat, 41, 41)))

    def forward(self, input):
        attn = F.interpolate(self.attn_lr, (input.size(2), input.size(3)), mode='bilinear', align_corners=False)
        return input * attn
...
if __name__ == '__main__':
    model = RFDNSW(conv='BSConvU', num_block=6)
    input = torch.rand(1, 3, 510, 339)
    output = model(input)
