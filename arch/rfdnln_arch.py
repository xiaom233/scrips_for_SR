...
class RFDB(nn.Module):
    ...
    def forward(self, input):
        ...
        out_fused = out_fused.permute(0, 3, 1, 2) + input
        out_fused = out_fused.flatten(2).transpose(1, 2)
        out_fused = self.norm(out_fused)
        out_fused = out_fused.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        return out_fused
...
if __name__ == '__main__':
    input = torch.rand(1, 3, 21, 21)
    model = RFDNLN(conv='BSConvU', num_block=6)
    output = model(input)