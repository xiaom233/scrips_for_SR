...
class RFDNFINALB12(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        super(RFDNFINALB12, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
        ...
        self.B7 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B8 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B9 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B10 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B11 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B12 = RFDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
    ...
    def forward(self, input):
        ...
        out_B8 = self.B7(out_B7)
        out_B9 = self.B5(out_B8)
        out_B10 = self.B6(out_B9)
        out_B11 = self.B7(out_B10)
        out_B12= self.B7(out_B11)

        trunk = torch.cat([out_B1, out_B2, out_B3, out_B4, out_B5, out_B6, out_B7, out_B8, out_B9, out_B10, out_B11, out_B12], dim=1)
