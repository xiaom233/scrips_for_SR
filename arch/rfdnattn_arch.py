...
class RAB(nn.Module):
    def __init__(self, in_channels=50, out_channels=50, kernel_size=3, conv=Blocks.BSConvU, p=0.25):
        super(RAB, self).__init__()
        kwargs = {'padding': 1}
        if conv.__name__ == 'BSConvS':
            kwargs = {'p': p}
        self.conv = conv(in_channels, out_channels, kernel_size=kernel_size,  **kwargs)

    def forward(self, input):
        fea = self.conv(input)
        return torch.mul(fea, input)
...
class RFDB(nn.Module):
    def __init__(self, in_channels, out_channels, distillation_rate=0.25, conv=nn.Conv2d, p=0.25):
        ...
        self.c1_r = RAB(in_channels, self.rc, kernel_size=3)
        self.c2_d = nn.Linear(self.remaining_channels, self.dc)
        self.c2_r = RAB(self.remaining_channels, self.rc, kernel_size=3)
        self.c3_d = nn.Linear(self.remaining_channels, self.dc)
        self.c3_r = RAB(self.remaining_channels, self.rc, kernel_size=3)
