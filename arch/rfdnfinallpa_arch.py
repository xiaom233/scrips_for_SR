...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.25):
        ...
        self.sigmoid = nn.Sigmoid()
        self.GELU = nn.LeakyReLU(negative_slope=0.18)
...
class RFDB(nn.Module):
    def __init__(self, in_channels, out_channels, distillation_rate=0.25, conv=nn.Conv2d, p=0.25):
        ...
        self.act = nn.LeakyReLU(negative_slope=0.18)

        self.c5 = nn.Linear(self.dc * 4, in_channels)
        self.attn = LPA(in_channels, conv)
...
class RFDNFINALLPA(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        super(RFDNFINALLPA, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
    ...
    def forward(self, input):
        ...
        out_B = self.act(out_B.permute(0, 3, 1, 2))
        # print(out_B.shape)
