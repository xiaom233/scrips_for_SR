...
class RFDB(nn.Module):
    def __init__(self, in_channels, out_channels, distillation_rate=0.25, conv=nn.Conv2d, p=0.25):
        ...
        # self.last1 = conv(in_channels, in_channels, kernel_size=3, **kwargs)
        # self.last2 = conv(in_channels, in_channels, kernel_size=3, **kwargs)
        # self.last3 = conv(in_channels, in_channels, kernel_size=3, **kwargs)
    ...
    def forward(self, input):
        ...
        out_fused = self.c5(out).permute(0, 3, 1, 2)
        # out_fused = self.conv_out(out).permute(0, 3, 1, 2)
