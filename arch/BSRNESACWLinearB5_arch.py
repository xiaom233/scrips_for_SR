...
class ESDB(nn.Module):
    ...
    def forward(self, input):
        ...
        out = self.c5(out)
        out_fused = self.esa(out).permute(0, 2, 3, 1) * self.cw
        out_fused = self.conv_out(out_fused.permute(0, 3, 1, 2))
...
class BSRNESACWLinearB5(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=64, num_block=8, num_out_ch=3, upscale=4,
        ...
        super(BSRNESACWLinearB5, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
