...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.45):
        ...
        self.conv2 = conv(f, f, kernel_size=3, stride=2, padding=0, **BSConvS_kwargs)
...
class ESDB(nn.Module):
    def __init__(self, in_channels, out_channels, conv=nn.Conv2d, p=0.45):
...
class BSRNB4(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
                 conv='BSConvU', upsampler='pixelshuffledirect', p=0.45):
        super(BSRNB4, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
