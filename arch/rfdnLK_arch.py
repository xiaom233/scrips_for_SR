...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.25):
        ...
        f = num_feat // 4
        kwargs = {'padding': 2, 'kernel_size': 5}
        if conv.__name__ == 'BSConvS':
            kwargs = {'p': p, 'padding': 2, 'kernel_size': 5}
        self.conv1 = nn.Linear(num_feat, f)
        self.conv_f = nn.Linear(f, f)
        self.maxPooling = nn.MaxPool2d(kernel_size=7, stride=3)
        self.conv_max = conv(f, f, **kwargs)
        self.conv2 = nn.Conv2d(f, f, 3, 2, 0)
        self.conv3 = conv(f, f, **kwargs)
        self.conv3_ = conv(f, f, **kwargs)
...
class RFDB(nn.Module):
    def __init__(self, in_channels, out_channels, distillation_rate=0.25, conv=nn.Conv2d, p=0.25):
        ...
        self.c1_r = conv(in_channels, self.rc, **kwargs)
        self.c2_d = nn.Linear(self.remaining_channels, self.dc)
        self.c2_r = conv(self.remaining_channels, self.rc,  **kwargs)
        self.c3_d = nn.Linear(self.remaining_channels, self.dc)
        self.c3_r = conv(self.remaining_channels, self.rc, **kwargs)

        self.c4 = conv(self.remaining_channels, self.dc, **kwargs)
        self.act = nn.GELU()
...
class RFDNLK(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        if conv == 'DepthWiseConv':
            ...
            self.conv = nn.Conv2d
        self.fea_conv = self.conv(num_in_ch, num_feat, **kwargs)
        ...
        self.c2 = self.conv(num_feat, num_feat, **kwargs)
