...
class RFDNREP(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
                 conv='DepthWiseConv', upsampler='pixelshuffledirect', p=0.25, replicate=4):
        super(RFDNREP, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
        if conv == 'DepthWiseConv':
            ...
            self.conv = nn.Conv2d
        self.fea_conv = self.conv(num_in_ch * replicate, num_feat, kernel_size=3, **kwargs)
