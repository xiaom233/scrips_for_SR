...
class ImageNetPairedDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # lq_path = lq_path[:-5] + '.png'
        if self.opt['phase'] != 'train':
            img_bytes = self.file_client.get(lq_path, 'lq')
            img_lq = imfrombytes(img_bytes, float32=True)

        # modcrop and resize for small size pairs.
        if self.opt['phase'] == 'train':
            ...
            # if size_h >= self.opt['gt_size'] and size_w >= self.opt['gt_size']:
            #     img_bytes = self.file_client.get(lq_path, 'lq')
            #     img_lq = imfrombytes(img_bytes, float32=True)
            # else:
            #     size_h = int(max(size_h, self.opt['gt_size']))
            #     size_w = int(max(size_w, self.opt['gt_size']))
