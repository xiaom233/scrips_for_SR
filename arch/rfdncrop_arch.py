...
class RFDB(nn.Module):
    def __init__(self, in_channels, out_channels, distillation_rate=0.25, conv=nn.Conv2d, p=0.25):
        ...
        self.act = nn.GELU()
        self.cw = nn.Parameter(torch.ones(1, self.rc * 4))
        self.c5 = nn.Linear(self.rc * 4, in_channels)
        self.esa = ESA(in_channels, conv)
        self.conv_out = nn.Linear(in_channels, out_channels)

    def forward(self, input):

        distilled_c1 = self.act(input)
        r_c1 = self.c1_r(input)
        r_c1 = self.act(r_c1 + input)

        distilled_c2 = self.act(r_c1)
        r_c2 = self.c2_r(r_c1)
        r_c2 = self.act(r_c2 + r_c1)

        distilled_c3 = self.act(r_c2)
        r_c3 = self.c3_r(r_c2)
        ...
        out = self.cw * out.permute(0, 2, 3, 1)
