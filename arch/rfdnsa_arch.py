...
class AttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.conv1(attn.permute(0, 2, 3, 1))

        return u * attn.permute(0, 3, 1, 2)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        shorcut = x.clone()
        x = self.proj_1(x.permute(0, 2, 3, 1))
        x = self.activation(x)
        x = self.spatial_gating_unit(x.permute(0, 3, 1, 2))
        x = self.proj_2(x.permute(0, 2, 3, 1))
        x = x.permute(0, 3, 1, 2) + shorcut
        return x
